...
const VideoComponent = ({ like, videoLink, imgLink }: cardI) => {
  ...
  let randomId = 'heart' + Math.random() * 10
  ...
  return (
    ...
    >
      ...
      <label className='absolute z-20 top-4 right-4 cursor-pointer' htmlFor={randomId.toString()}>
        <input className='sr-only peer [&:checked+svg]:!fill-red-500' type="checkbox" id={randomId.toString()} />
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" strokeWidth={1.5} stroke="white" className="size-8 pointer-events-none fill-black/40">
