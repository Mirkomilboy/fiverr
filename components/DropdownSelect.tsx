  ...
  return (
    <div ref={dropdownRef} className="relative">
      <button
        ...
        <span>{title}</span>
        <svg
          className={`size-3 ${id == open ? "rotate-180" : ""} transition-transform duration-200`}
          viewBox="0 0 11 7"
      ...
      <Transition
        ...
        className="max-w-xl min-w-max w-full shrink-0 bg-white border border-100 rounded-lg shadow-dropdown overflow-hidden absolute mt-2">
        <div className="rounded-lg min-h-20 max-h-[42rem]  overflow-y-scroll h-full divide-y divide-gray-100 z-50 px-6">
          {data.map((item: any, idx: any) => (
            <SelectOptionsContent key={idx} data={item} />
          ))}
        </div>
